...
export function FinancePage() {
  ...
  const canReadOwn = authEnabled && hasPermission('cost_centers:read_own');
  const canReadAllFinance = authEnabled && hasPermission('cost_centers:read_all');
  const canCreateCostCenters = authEnabled && hasPermission('cost_centers:create');
  const canUpdateCostCenters = authEnabled && hasPermission('cost_centers:modify');
  const canUpdateBudgets = authEnabled && hasPermission('cost_centers:modify');
  const canAssignCostCenterUsers = authEnabled && hasPermission('cost_centers:modify');
  const canAdjustWallet = authEnabled && hasPermission('cost_centers:modify');
  const canReadUsers = authEnabled && hasPermission('users:read');
  ...
  const handleSaveManualPrint = async () => {
    ...
    // The API accepts a positive amount and records manual print charges as
    // negative ledger entries itself. Keep the UI request aligned with that
    // contract instead of sending a value that Pydantic rejects.
