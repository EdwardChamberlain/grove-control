/**
 * Card and row actions must stay reachable without a hover-capable pointer (#86).
 *
 * Tailwind v4 compiles `group-hover:` inside `@media (hover: hover)`, so a
 * bare `opacity-0` hides these controls permanently on touch-only devices.
 * jsdom does not evaluate media queries, so these tests pin the class contract
 * and verify that the controls remain tappable.
 */
...
describe('actions that were hover-only (#86)', () => {
  afterEach(() => {
    setAuthToken(null);
  });
