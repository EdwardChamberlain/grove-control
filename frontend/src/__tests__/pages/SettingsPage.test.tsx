  ...
  describe('general settings', () => {
    ...
    it('saves mixed Sidebar order when items are dragged', async () => {
      ...
      expect(localStorage.setItem).toHaveBeenCalledWith(
        SIDEBAR_ORDER_KEY,
        JSON.stringify(['ext-7', 'printers', 'inventory', 'archives', 'queue', 'projects', 'finance', 'files', 'makerworld', 'profiles', 'maintenance', 'stats', 'settings']),
      );
    });
    ...
    it('resets Sidebar to all pages first and configured links at the bottom', async () => {
      ...
      expect(localStorage.setItem).toHaveBeenCalledWith(
        SIDEBAR_ORDER_KEY,
        JSON.stringify(['printers', 'inventory', 'archives', 'queue', 'projects', 'finance', 'files', 'makerworld', 'profiles', 'maintenance', 'stats', 'settings', 'ext-7']),
      );
