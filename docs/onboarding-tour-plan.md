# Grove Control Onboarding Tour - Detailed Plan
...
2. Walk a brand-new user from zero to "first print sent via Grove Control" in under 10 minutes.
3. Surface all major Grove Control features at the right depth (overview, not full docs) so users discover what's available without reading the wiki cover-to-cover.
...
- Headline: "Hallo! Welcome to Grove Control."
- Body: "Grove Control replaces the Bambu Lab cloud with a local-first dashboard. Your data, prints, spools, and timelapses stay on your hardware. Want a 5-minute tour?"
- Three buttons:
...
### Step 0.2 - What Grove Control is (and isn't)
**Anchor:** modal, BB "Let me walk you through it" pose (2)
**Content:**
- Two-column comparison:
  - **What Grove Control does:** local cloud replacement, AMS + inventory + RFID, print queue, archives, slicer integration via Virtual Printer, multi-user, HomeAssistant, optional Tailscale.
  - **What Grove Control isn't (today):** not a slicer (uses BambuStudio/OrcaSlicer), not a cloud service, not a printer firmware tool, not a Klipper UI.
...
- Body: "Grove Control can run with or without authentication. If anyone else on your network (or your tailnet, or your reverse proxy) can reach this URL, turn auth on now — passwords, OIDC, SAML, and MFA are all built in."
- Inline severity callout (yellow): "Grove Control can also control your printers, manage files, and read your camera feeds. Treat the URL like an admin panel."
**Buttons:**
...
- Headline: "Let's make sure Grove Control can talk to it"
...
2. **AMS row** — slots, RFID auto-detection, drying button. "Grove Control reads your AMS slot config live — colors and types come from RFID, the rest from your inventory."
...
- Body: "Grove Control can keep tabs on your spools. Pick a mode now — switching later loses data."
- Three large radio cards:
  ...
  - **Spoolman** — point at an existing Spoolman instance, Grove Control syncs from it. Best if you already run Spoolman.
...
- "Add a spool the way that suits you:"
  - **RFID scan** (Bambu spools) — load it in the AMS, Grove Control detects automatically. No manual entry needed.
...
- One-line note: "Grove Control ships with a color catalog covering the major brands — names autocomplete as you type."
...
**Content:**
- "Tell Grove Control where Spoolman lives."
- Inline form: Spoolman URL + sync direction (Spoolman → Grove Control, or bi-directional).
- "Grove Control will pull your existing spool library and keep it in sync. RFID scans still work — they create new spools in Spoolman."
...
- Body: "If you've created custom filament or print profiles in BambuStudio + the Bambu cloud, Grove Control can pull them down so they're available everywhere — assigned via the web UI, sent via Virtual Printer, used by the queue."
- Inline form: Bambu Lab account email + password (or "Sign in later")
- One-line warning: "Grove Control stores credentials encrypted at rest and only uses them against the official Bambu API. Source is open."
...
- Three things the queue can do, with a one-line example each:
  ...
  2. **Auto-dispatch** — Grove Control assigns queued jobs to idle printers automatically based on AMS / build-plate / capacity.
...
**Content:**
- BB "Helpful" pose
- "Grove Control tracks consumables and maintenance per printer."
...
**Content:**
- "Want BambuStudio / OrcaSlicer to send prints to Grove Control instead of the cloud?"
- Four-mode decision tree (one sentence each):
  - **Bridge** — drop-in cloud replacement; slicer sends, Grove Control forwards to the real printer.
  - **Queue** — slicer sends to a virtual collector; Grove Control queues for dispatch.
  - **Proxy** — slicer points at Grove Control, Grove Control passes through with full MQTT/FTP/RTSP rewrite (best for multi-slicer setups).
  - **Archive / Review** — slicer sends, Grove Control stores but doesn't print. Audit / approval workflows.
...
**Content:**
- "Slice directly inside Grove Control from MakerWorld URLs or your library — no BambuStudio needed."
- "Requires the orca-slicer-api sidecar container (separate docker-compose, link below). Grove Control talks to it over HTTP."
...
**Content:**
- "Mount a NAS share, an external SSD, or a project drive — Grove Control reads files in-place."
- "Set `BAMBUDDY_EXTERNAL_ROOTS` in `docker-compose.yml`, bind-mount the host path. Grove Control auto-shows folders in the File Manager."
...
**Content:**
- "Paste any MakerWorld URL — Grove Control downloads the 3MF, adds it to your library."
...
- "Grove Control talks directly to your self-hosted Obico ML server. Opt-in per printer; off by default."
...
**Content:**
- "Grove Control ships first-class HomeAssistant integration — sensors for every printer (state, temp, ETA, AMS slots), services to start / pause / cancel."
...
**Content:**
- "Access Grove Control from anywhere via your tailnet — no port forwarding, no public exposure."
- "MagicDNS HTTPS via Let's Encrypt (Grove Control requests certs via `tailscale cert`)."
- One-line note: "Read the Tailscale blog post about Grove Control at [link] for the full setup walkthrough."
...
**Content:**
- "Group users by role. Grove Control ships with default groups: Admin, Operator, Viewer."
...
**Content:**
- "Grove Control supports OIDC (Authentik, Authelia, Keycloak, Google, GitHub) and SAML 2.0 for org SSO."
...
| Slicer-side mistaken as Grove Control | 5.1 → "is this Grove Control?" | #1597, #1525, #1582, #1579, #1578 |
