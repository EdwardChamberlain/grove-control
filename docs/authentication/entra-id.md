...
This guide shows how to configure Grove Control's OIDC integration with **Microsoft Azure Entra ID** (formerly Azure Active Directory).
...
- Grove Control ≥ 1.0 with OIDC enabled (Settings → Authentication → OIDC Providers).
...
## 4. Add the Provider in Grove Control
...
- **`preferred_username`** (recommended) — Entra ID always populates this with the UPN (e.g. `user@contoso.com`). Grove Control treats it as an email-shaped identifier and skips the `email_verified` check entirely.
...
Azure Entra ID issues access tokens that expire after 1 hour by default. Grove Control exchanges the OIDC code for its own JWT at callback time, so the Entra token lifetime does not affect the Grove Control session length. You can adjust Grove Control's `JWT_ACCESS_TOKEN_EXPIRE_MINUTES` env var independently.
...
|---|---|
| Login redirects to Entra but returns "OIDC login failed" | Redirect URI mismatch — check the URI registered in Azure exactly matches Grove Control's callback URL, including scheme and trailing path. |
...
| "Invalid client" error from Azure | Client secret has expired or was copied incorrectly. Rotate the secret in Azure and update the provider in Grove Control. |
