#!/bin/bash
#
# Docker Test Suite for Grove Control
...
print_header "Grove Control Docker Test Suite"
...
if [ "$RUN_BUILD" = true ]; then
    ...
    if sudo docker build -t grove-control:test . $CACHE_FLAG --progress=plain; then
        ...
        if sudo docker run --rm grove-control:test python -c "import backend.app.main; print('Backend imports OK')"; then
            print_success "Backend module imports correctly"
        else
            print_failure "Backend module import failed"
        fi

        if sudo docker run --rm grove-control:test test -d /app/static; then
