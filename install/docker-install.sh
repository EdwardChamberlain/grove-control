#!/usr/bin/env bash
#
# Grove Control Docker Installation Script
# Supports: Linux (all distros), macOS
#
# Usage:
#   Interactive:  curl -fsSL https://raw.githubusercontent.com/EdwardChamberlain/grove-control/main/install/docker-install.sh -o docker-install.sh && chmod +x docker-install.sh && ./docker-install.sh
...
show_help() {
    echo "Grove Control Docker Installation Script"
    echo ""
...
download_compose_file() {
    ...
    if [[ "$BUILD_FROM_SOURCE" == "true" ]]; then
        ...
        curl -fsSL -o docker-compose.yml \
            https://raw.githubusercontent.com/EdwardChamberlain/grove-control/main/docker-compose.yml
    fi
...
create_env_file() {
    ...
    cat > .env << EOF
# Grove Control Docker Configuration
# Generated by docker-install.sh on $(date)

# Port Grove Control runs on
PORT=$PORT
...
start_container() {
    log_info "Starting Grove Control..."
    ...
    while [[ $attempt -lt $max_attempts ]]; do
        ...
        if $DOCKER_CMD ps | grep -q "Up"; then
            log_success "Grove Control container is running"
            return 0
        fi
        ...
        if $DOCKER_CMD ps -a | grep -q "Exited"; then
            ...
            log_info "Check logs with: $DOCKER_CMD logs grove-control"
            return 1
        fi
...
main() {
    ...
    echo -e "  ${BOLD}Access Grove Control:${NC}  ${CYAN}http://localhost:$PORT${NC}"
    ...
    echo -e "    Logs:    cd $INSTALL_PATH && $DOCKER_CMD logs -f grove-control"
    ...
    echo ""
    echo -e "  ${BOLD}Update Grove Control:${NC}"
    if [[ "$BUILD_FROM_SOURCE" == "true" ]]; then
