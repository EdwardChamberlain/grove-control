<#
.SYNOPSIS
    Grove Control Docker installation script for Windows (Docker Desktop).
...
function Start-Grove Control {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Host "    $DockerCompose logs grove-control" -ForegroundColor Yellow
        exit 1
    }
...
Write-Host "    Logs:     cd `"$InstallPath`"; $DockerCompose logs -f grove-control"
