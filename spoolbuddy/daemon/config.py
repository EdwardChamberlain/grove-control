...
Required:
    SPOOLBUDDY_BACKEND_URL  — Grove Control server URL (e.g. http://192.168.1.100:5000)
    SPOOLBUDDY_API_KEY      — API key created in Grove Control Settings → API Keys
...
class Config:
    ...
    def load(cls) -> "Config":
        ...
        if not cfg.api_key:
            raise RuntimeError("SPOOLBUDDY_API_KEY is required (create one in Grove Control Settings → API Keys)")
