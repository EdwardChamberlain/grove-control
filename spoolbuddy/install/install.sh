...
#   1) SpoolBuddy only — NFC/scale companion connecting to a remote Grove Control instance
#   2) SpoolBuddy + Grove Control — both running natively on this Raspberry Pi
#
# Usage:
#   Interactive:  curl -fsSL https://raw.githubusercontent.com/EdwardChamberlain/grove-control/main/spoolbuddy/install.sh -o install.sh && chmod +x install.sh && sudo ./install.sh
...
#   --bambuddy-url URL   Grove Control server URL (required for spoolbuddy mode)
#   --api-key KEY        Grove Control API key (required for spoolbuddy mode)
#   --path PATH          Installation directory (default: /opt/spoolbuddy or /opt/bambuddy)
#   --port PORT          Grove Control port (full mode only, default: 8000)
#   --ssh-pubkey KEY     Grove Control SSH public key for remote updates
...
GITHUB_REPO="https://github.com/EdwardChamberlain/grove-control.git"
...
SSH_PUBKEY=""            # Grove Control's SSH public key for remote updates
...
show_help() {
    ...
    echo ""
    echo "Options:"
    echo "  --mode MODE          \"spoolbuddy\" (companion only) or \"full\" (Grove Control + SpoolBuddy)"
    echo "  --repo URL           Git repository URL to install from"
    echo "  --ref REF            Git ref to install (branch/tag/commit)"
    echo "  --bambuddy-url URL   Grove Control server URL (required for spoolbuddy mode)"
    echo "  --api-key KEY        Grove Control API key (required for spoolbuddy mode)"
    echo "  --path PATH          Installation directory (default: /opt/spoolbuddy or /opt/bambuddy)"
    echo "  --port PORT          Grove Control port (full mode only, default: 8000)"
    echo "  --ssh-pubkey KEY     Grove Control SSH public key for remote updates"
...
create_spoolbuddy_user() {
    ...
    # Allow passwordless restart of daemon + kiosk (needed for SSH-based updates from Grove Control)
...
# Grove Control backend URL
SPOOLBUDDY_BACKEND_URL=$BAMBUDDY_URL

# API key (create one in Grove Control Settings -> API Keys)
...
setup_ssh_key() {
    info "Setting up SSH access for Grove Control remote updates..."
    ...
    if [[ -n "$SSH_PUBKEY" ]]; then
        ...
        info "SSH key will be deployed automatically when the daemon connects to Grove Control"
        touch "$auth_keys"
    fi
...
Documentation=https://github.com/EdwardChamberlain/grove-control
$after_line
...
# ─────────────────────────────────────────────────────────────────────────────
# Grove Control Installation (full mode only)
# ─────────────────────────────────────────────────────────────────────────────
...
setup_bambuddy_venv() {
    ...
    run_with_progress "Creating Grove Control venv" $PYTHON_CMD -m venv venv
    run_with_progress "Upgrading pip" "$INSTALL_PATH/venv/bin/pip" install --upgrade pip
    run_with_progress "Installing Grove Control dependencies" "$INSTALL_PATH/venv/bin/pip" install -r requirements.txt
...
create_bambuddy_env() {
    info "Creating Grove Control configuration..."

    local env_file="$INSTALL_PATH/.env"

    cat > "$env_file" << EOF
# Grove Control Configuration
# Generated by install.sh on $(date)
...
create_bambuddy_service() {
    info "Creating Grove Control systemd service..."

    cat > /etc/systemd/system/bambuddy.service << EOF
[Unit]
Description=Grove Control - Bambu Lab Print Management
Documentation=https://github.com/EdwardChamberlain/grove-control
...
EOF
    ...
    success "Grove Control service created and enabled"
}

bootstrap_spoolbuddy_kiosk_key() {
    # Provision an API key for the local SpoolBuddy kiosk and write it into
    # spoolbuddy/.env. Runs against the Grove Control DB directly (via the CLI),
...
setup_kiosk() {
    ...
    if [[ -f "$boot_config" ]]; then
        ...
        if ! grep -q "^disable_splash=" "$boot_config"; then
            echo "" >> "$boot_config"
            echo "# Kiosk: Disable Raspberry Pi firmware splash, use custom grove_control_spoolbuddy_splash.png" >> "$boot_config"
    ...
    local script_dir
    script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    if [[ -f "$script_dir/grove_control_spoolbuddy_splash.png" ]]; then
        cp "$script_dir/grove_control_spoolbuddy_splash.png" "$theme_dir/grove_control_spoolbuddy_splash.png"
    elif [[ -f "$INSTALL_PATH/spoolbuddy/install/grove_control_spoolbuddy_splash.png" ]]; then
        cp "$INSTALL_PATH/spoolbuddy/install/grove_control_spoolbuddy_splash.png" "$theme_dir/grove_control_spoolbuddy_splash.png"
    else
        warn "grove_control_spoolbuddy_splash.png not found — Plymouth splash will not display an image"
    fi
...
wallpaper_image = Image("grove_control_spoolbuddy_splash.png");
...
# Wait for the Grove Control backend to be reachable before launching Chromium.
...
ask_install_mode() {
    ...
    echo "     NFC reader + scale on this RPi, Grove Control runs on another device"
    echo ""
    echo -e "  ${CYAN}2)${NC} SpoolBuddy + Grove Control"
...
gather_config() {
    ...
    if [[ "$INSTALL_MODE" == "spoolbuddy" ]]; then
        # Need remote Grove Control URL and API key
        echo ""
        info "SpoolBuddy needs to connect to your Grove Control server."
        info "You can find/create an API key in Grove Control under Settings -> API Keys."
        echo ""

        while [[ -z "$BAMBUDDY_URL" ]]; do
            prompt "Grove Control server URL (e.g. http://192.168.1.100:8000)" "" BAMBUDDY_URL
            if [[ -z "$BAMBUDDY_URL" ]]; then
                warn "Grove Control URL is required"
            fi
        done

        while [[ -z "$API_KEY" ]]; do
            prompt "Grove Control API key" "" API_KEY
            if [[ -z "$API_KEY" ]]; then
                warn "API key is required"
            fi
        done
    else
        # Full mode — Grove Control runs locally
        prompt "Grove Control port" "$BAMBUDDY_PORT" BAMBUDDY_PORT
        BAMBUDDY_URL="http://localhost:$BAMBUDDY_PORT"

        echo ""
        info "After installation, create an API key in Grove Control (Settings -> API Keys)"
    ...
    echo -e "${CYAN}─────────────────────────────────────────${NC}"
    echo -e "  Mode:           ${GREEN}$([ "$INSTALL_MODE" == "full" ] && echo "Grove Control + SpoolBuddy" || echo "SpoolBuddy only")${NC}"
    echo -e "  Install path:   ${GREEN}$INSTALL_PATH${NC}"
    echo -e "  Git repo:       ${GREEN}$INSTALL_REPO${NC}"
    echo -e "  Git ref:        ${GREEN}$INSTALL_REF${NC}"
    if [[ "$INSTALL_MODE" == "full" ]]; then
        echo -e "  Grove Control port:  ${GREEN}$BAMBUDDY_PORT${NC}"
        echo -e "  Grove Control URL:   ${GREEN}$BAMBUDDY_URL${NC}"
    else
        echo -e "  Grove Control URL:   ${GREEN}$BAMBUDDY_URL${NC}"
    fi
    echo ""
...
main() {
    ...
    echo -e "${CYAN}║                                                          ║${NC}"
    echo -e "${CYAN}║          NFC Spool Management for Grove Control               ║${NC}"
    echo -e "${CYAN}║                                                          ║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    # ── Step 5: Grove Control setup (full mode only) ───────────────────────────
    if [[ "$INSTALL_MODE" == "full" ]]; then
        info "Setting up Grove Control..."
        create_bambuddy_user
        setup_bambuddy_venv
        install_nodejs
        build_frontend
    ...
    if [[ "$INSTALL_MODE" == "full" ]]; then
        echo -e "  ${BOLD}Grove Control:${NC}         ${CYAN}http://$ip_addr:$BAMBUDDY_PORT${NC}"
    else
    ...
    if [[ "$INSTALL_MODE" == "full" ]]; then
        echo -e "    Grove Control status:     ${CYAN}sudo systemctl status bambuddy${NC}"
        echo -e "    Grove Control logs:       ${CYAN}sudo journalctl -u bambuddy -f${NC}"
    fi
