...
scan_codeql_python() {
    ...
    gh codeql database create --overwrite --language=python --threads=0 /tmp/grove-control-codeql-python &>/dev/null
    echo "Analyzing..."
    gh codeql database analyze /tmp/grove-control-codeql-python \
        "$PROJECT_ROOT/.codeql/python-grove-control.qls" \
...
scan_codeql_js() {
    ...
    echo "Creating database..."
    gh codeql database create --overwrite --language=javascript --source-root=frontend --threads=0 /tmp/grove-control-codeql-javascript &>/dev/null
    echo "Analyzing..."
    gh codeql database analyze /tmp/grove-control-codeql-javascript \
        "$PROJECT_ROOT/.codeql/javascript-grove-control.qls" \
        --threads=0 --format=sarifv2.1.0 --output="$sarif" &>/dev/null
    echo ""
    parse_sarif "$sarif"
}
...
scan_codeql_actions() {
    ...
    echo "Creating database..."
    gh codeql database create --overwrite --language=actions --threads=0 /tmp/grove-control-codeql-actions &>/dev/null
    echo "Analyzing..."
    gh codeql database analyze /tmp/grove-control-codeql-actions \
...
scan_trivy_image() {
    ...
    docker build -t grove-control:security-scan . 2>&1
    echo ""
    trivy image --severity CRITICAL,HIGH,MEDIUM grove-control:security-scan 2>&1
}
...
echo -e "${BOLD}Grove Control Security Scanner${NC}"
