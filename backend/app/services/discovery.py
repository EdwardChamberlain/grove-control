...
# Virtual printer serial suffix to exclude from discovery (Grove Control's own virtual printer)
...
class PrinterDiscoveryService:
    ...
    def _handle_response(self, response: str, ip_address: str):
        ...
        # Skip Grove Control's own virtual printer (any model variant)
        if serial.endswith(VIRTUAL_PRINTER_SERIAL_SUFFIX):
            logger.debug("Ignoring Grove Control virtual printer at %s", ip_address)
            return
...
class SubnetScanner:
    ...
    async def _probe_host(self, ip: str, timeout: float):
        ...
        if serial and serial.endswith(VIRTUAL_PRINTER_SERIAL_SUFFIX):
            logger.debug("Ignoring Grove Control virtual printer at %s", ip)
            return
