...
# the honest Grove Control/1.0 string so the application-layer identity stays
# truthful (TLS fingerprint matches Chrome because Python's TLS is the
# signal CF gates on; everything above TLS is still Grove Control).
#
...
# Map Grove Control model names to Bambu Lab API keys
MODEL_TO_API_KEY = {
    "X1": "x1",
    "X1C": "x1",
    "X1-Carbon": "x1",
...
class FirmwareCheckService:
    ...
    def _get_bambulab_client(self) -> object | None:
        ...
        Grove Control string so application-layer identity stays truthful.
    ...
    async def get_latest_version(self, model: str) -> FirmwareVersion | None:
        ...
        Args:
            model: Grove Control printer model name (e.g., "X1C", "P1S", "H2D")
