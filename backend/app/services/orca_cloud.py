...
publishable key. Grove Control generates the verifier/challenge/state, redirects the
...
callback URL back into Grove Control (the loopback URL is the only ``redirect_to``
...
honest ``Grove Control/<version>`` UA clears it. No TLS-fingerprint matching needed.
"""
...
# Honest client identity. Same posture as Bambu Cloud: identifies Grove Control
...
def generate_pkce() -> tuple[str, str, str]:
    ...
    handshake. The verifier is the secret kept by Grove Control until the code
...
class OrcaCloudService:
    ...
    async def password_login(self, email: str, password: str) -> dict[str, Any]:
        ...
        allows). The actual server behaviour is what matters for Grove Control
    ...
    async def get_user_info(self) -> dict[str, Any]:
        ...
        for display in Grove Control's UI."""
