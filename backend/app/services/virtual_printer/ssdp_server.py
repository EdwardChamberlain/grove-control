...
Also provides SSDP proxy functionality for proxy mode, where Grove Control sits
...
class SSDPProxy:
    ...
    header changed to point to Grove Control's IP on LAN B.

    This allows Bambu Studio on LAN B to discover the printer via Grove Control.
    """
    ...
    def _rewrite_ssdp(self, data: bytes) -> bytes:
        ...
        - Location: changed to Grove Control's remote interface IP
    ...
    async def _broadcast_to_remote(self) -> None:
        ...
        try:
            # Rewrite Location to point to Grove Control's remote interface
            rewritten = self._rewrite_ssdp(self._last_printer_ssdp)
