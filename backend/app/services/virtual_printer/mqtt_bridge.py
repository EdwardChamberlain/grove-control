...
without opening a second MQTT session on the printer (reuses Grove Control's
...
Identity rewriting at cache time:
  ...
  - `net.info[*].ip` little-endian uint32 → VP bind IP. BambuStudio reads
    ...
    to the real printer and bypasses Grove Control.
...
# Grove Control's internal printer state in bambu_mqtt.py (around line 2686+) is
...
def _resolve_host_interface_for_target(target_ip: str) -> str | None:
    ...
    resolves to Grove Control rather than the real printer. Returns the IPv4 of
...
def _merge_ams_dict(prev_ams: dict, new_ams: dict) -> dict:
    ...
    2. Status-only incremental: ``{ams_status: 1}`` or ``{humidity: 30}`` —
       no ``ams`` array at all. Grove Control logs these as "AMS partial update
...
class MQTTBridge:
    ...
    def _resolve_client(self) -> None:
        ...
        # cache populates immediately. Grove Control itself queries these on connect,
    ...
    def _rewrite_net_info_ips(self, print_state: dict) -> int:
        ...
        IPs — only one matches the IP Grove Control tracks, but the slicer may
    ...
    def _on_printer_raw(self, topic: str, payload: bytes) -> None:
        ...
        if isinstance(print_data, dict) and print_data.get("command") == "push_status":
            ...
            # cache mirrors what Grove Control's AMS card shows internally. Without
        ...
        # the slicer (or Grove Control) issued; the slicer matches by sequence_id and
