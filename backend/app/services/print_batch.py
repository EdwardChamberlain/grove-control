...
# Statuses that consume a unit of the target. "dispatching" and "printing"
...
class PlateProgress:
    ...
    def can_dispatch(self) -> bool:
        """Whether an existing queue row can provide settings for another run."""
...
class BatchProgress:
    ...
    def dispatchable_remaining(self) -> int:
        """Runs still owed for plates that retain a clone source."""
...
async def refresh_batch_status(db: AsyncSession, batch: PrintBatch) -> bool:
    ...
    if batch.status == "active" and not progress.has_targets and progress.completed == 0:
        settled = progress.pending == 0 and progress.dispatching == 0 and progress.printing == 0
        ...
        (
            await db.execute(
                ...
                .where(
                    ...
                    .where(PrintQueueItem.status.in_(("pending", "dispatching", "printing")))
                    .exists()
                )
            )
        )
