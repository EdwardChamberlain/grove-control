    ...
    async def push_files(
        ...
        try:
            ...
            if err:
                ...
                logger.warning("push_files %s/%s: %s", owner, repo, msg)
                return {"status": "failed", "message": msg}
            commit_message = f"Grove Control backup - {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}"
    ...
    async def _create_initial_commit(
        ...
        try:
            ...
            commit_response = await client.post(
                ...
                headers=headers,
                json={
                    "message": f"Initial Grove Control backup - {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}",
                    "tree": tree_sha,
                },
            )
            if commit_response.status_code != 201:
