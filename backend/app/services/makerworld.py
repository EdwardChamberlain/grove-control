...
Lets Grove Control resolve a MakerWorld URL, enumerate plate/profile metadata, and
...
Grove Control's existing Bambu Cloud bearer token (same SSO backend — no separate
OAuth flow needed).
...
# as Grove Control with a source URL so Bambu can distinguish our traffic from
