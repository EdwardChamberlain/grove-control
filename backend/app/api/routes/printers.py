...
async def delete_printer(
    ...
    # Delete scheduled drying rows explicitly because SQLite does not enforce
...
async def set_fan_speed(
    ...
    fan_ids = {"part": 1, "aux": 2, "aux2": 10, "chamber": 3}
    ...
    if fan == "aux2" and (not uses_exhaust_fan_label(printer.model) or client.state.left_aux_fan_speed is None):
        raise HTTPException(400, "Left auxiliary fan is not reported by this printer")
    if fan == "chamber" and uses_exhaust_fan_label(printer.model) and not client.state.exhaust_fan_present:
        raise HTTPException(400, "Exhaust fan is not reported by this printer")
