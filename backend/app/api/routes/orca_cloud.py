...
Auth shape (see :mod:`backend.app.services.orca_cloud` for the deep dive):
    ...
    POST /orca-cloud/logout
        ...
        survives until its 1h expiry, but Grove Control has no way to use it).
...
async def auth_password(
    ...
    Grove Control POSTs the credentials to Supabase and stores the returned
...
async def logout(
    ...
    either way, and Grove Control will no longer have it to use)."""
