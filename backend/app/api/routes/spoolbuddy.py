...
async def device_heartbeat(
    ...
    # Grove Control's keypair rotates (data dir wiped, container recreated, etc.) —
...
async def nfc_tag_scanned(
    ...
    Routes the lookup to the inventory backend Grove Control is configured for:
    ...
    follow the inventory mode they selected in Grove Control settings.
    """
...
async def update_spool_weight(
    ...
    Routes the update to whichever inventory backend Grove Control is configured
...
async def check_daemon_update(
    ...
    """Check if the SpoolBuddy daemon needs updating to match the Grove Control backend version."""
...
async def trigger_daemon_update(
    ...
    Grove Control SSHes into the device, pulls the matching branch, installs deps,
