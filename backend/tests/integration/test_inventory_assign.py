...
class TestAssignSpoolEmptySlotPreConfig:
    ...
    deferred weigh-then-assign-before-insert case where state/tray_type
    ...
    async def test_on_ams_change_fires_config_when_pre_assigned_slot_loads(
        ...
        # Pre-existing assignment with empty fingerprint (the deferred state)
    ...
    async def test_on_ams_change_fires_replay_when_tray_type_appears_without_state_11(
        ...
        """A1 Mini / P1S firmware variant of the pre-config replay
        (#1322). The user pre-assigned via another client (fingerprint empty), then
...
class TestAssignSpoolEmptyDetection:
    ...
    async def test_state_empty_skips_mqtt_and_marks_pending(
        ...
        assert response.status_code == 200
        # Weigh-then-assign workflow: firmware drops MQTT for
