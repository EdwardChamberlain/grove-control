...
translates between Spoolman's data model and Grove Control's InventorySpool format.
"""
...
class TestSpoolmanInventorySlicerFilament:
    ...
    Spoolman has no native slicer_filament field — Grove Control persists the
...
class TestSpoolmanInventoryInputValidation:
    ...
    async def test_ssrf_blocked_schemes_and_addresses(
        ...
        topologies for self-hosted Grove Control deployments."""
        from backend.app.models.settings import Settings
    ...
    async def test_ssrf_allows_lan_spoolman_topologies(
        ...
        """Regression: Grove Control's normal deployment is LAN-local Spoolman.
...
class TestColorNamePassthrough:
    ...
    Spoolman 0.23.1 has no `color_name` field on Filament, so Grove Control owns
