...
async def test_client_headers_identify_honestly_and_send_browser_accept():
    ...
    must identify as Grove Control at the HTTP layer and must send Accept +
...
async def test_bambulab_curl_cffi_session_keeps_honest_user_agent():
    ...
    HTTP-layer User-Agent MUST stay 'Grove Control/...'. A future refactor that
    ...
    assert ua.startswith("Grove Control/"), f"curl_cffi session UA leaked Chrome default: {ua!r}"
