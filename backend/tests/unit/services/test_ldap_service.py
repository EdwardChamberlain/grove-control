...
class TestResolveGroupMapping:
    """Verify LDAP group DN to Grove Control group name resolution."""
...
class TestAuthenticateLdapUserGroups:
    ...
    def test_primary_gidnumber_group_found(self, mock_ldap):
        ...
        operators_group = _MockEntry("cn=grove-control-operators,ou=groups,dc=test,dc=com")
        ...
        assert info.groups == ["cn=grove-control-operators,ou=groups,dc=test,dc=com"]
    ...
    def test_dedupes_group_found_via_both_memberuid_and_primary_gid(self, mock_ldap):
        ...
        user_entry = _MockEntry("cn=mz,dc=test,dc=com", uid="mz", gidNumber=10002)
        group_entry = _MockEntry("cn=grove-control-operators,ou=groups,dc=test,dc=com")
    ...
    def test_case_insensitive_dedupe(self, mock_ldap):
        ...
        lower_dn = _MockEntry("cn=grove-control-operators,ou=groups,dc=test,dc=com")
    ...
    def test_no_gidnumber_skips_primary_search(self, mock_ldap):
        ...
        viewers_group = _MockEntry("cn=grove-control-viewers,ou=groups,dc=test,dc=com")
        ...
        assert info is not None
        assert info.groups == ["cn=grove-control-viewers,ou=groups,dc=test,dc=com"]
